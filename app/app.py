...
def toggle_app():
    ...
    st.button("Stopper l'app", on_click=stop_app, type="primary")
    st.write("💡 Vous pourrez toujours actualiser la page si elle s'éteint.")
    run()
    footer()
...
def init_app():
    if "is_running" not in st.session_state:
        ...
        st.button("Activer l'app", on_click=toggle_app, type="primary")
        footer()
...
# Store the start time in session_state so it persists across reruns
...
st.write(f"{elapsed_minutes:.2f} minutes avant extinction de l'app. Rafraichissez la page si elle se met en veille.")

# If more than 15 minutes have passed, stop execution
if elapsed_minutes >= 15:
    st.warning("⏱️ Session has reached the 15-minute limit. Stopping execution. Refresh the page.")
    stop_app()